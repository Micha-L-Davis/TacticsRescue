...
namespace Opertoon.Panoply {
	...
	public class PanoplyController: MonoBehaviour {
	    ...
	    public void UpdatePassiveInput() {
			...
			if (passiveInput) {
				...
				case PassiveInputType.Gyroscope:
					float angleDiff = (AttitudeSensor.current.attitude.ReadValue().eulerAngles.y - gyroCenter.x + 180) % 360 - 180;
					horizontalTilt = -Mathf.InverseLerp (-gyroRange.x, gyroRange.x, angleDiff);
					angleDiff = (AttitudeSensor.current.attitude.ReadValue().eulerAngles.x - gyroCenter.y + 180) % 360 - 180;
					verticalTilt = -Mathf.InverseLerp (-gyroRange.y, gyroRange.y, angleDiff);
					break;

				case PassiveInputType.Accelerometer:
					if (Accelerometer.current.acceleration.ReadValue().x < _accelerometerMin.x) {
						_accelerometerMin.x = Mathf.Min (_accelerometerMin.x, Accelerometer.current.acceleration.ReadValue().x);
					} else if (Accelerometer.current.acceleration.ReadValue().x > (_accelerometerMin.x + accelerometerRange.x)) {
						_accelerometerMin.x = Accelerometer.current.acceleration.ReadValue().x - accelerometerRange.x;
					}
					if (Accelerometer.current.acceleration.ReadValue().y < _accelerometerMin.y) {
						_accelerometerMin.y = Mathf.Min (_accelerometerMin.y, Accelerometer.current.acceleration.ReadValue().y);
					} else if (Accelerometer.current.acceleration.ReadValue().y > (_accelerometerMin.y + accelerometerRange.y)) {
						_accelerometerMin.y = Accelerometer.current.acceleration.ReadValue().y - accelerometerRange.y;
					}
					acceleration.x = (Mathf.InverseLerp (_accelerometerMin.x + accelerometerRange.x, _accelerometerMin.x, Accelerometer.current.acceleration.ReadValue().x) * 2.0f) - 1.0f;
					acceleration.y = (Mathf.InverseLerp (_accelerometerMin.y, _accelerometerMin.y + accelerometerRange.y, Accelerometer.current.acceleration.ReadValue().y) * 2.0f) - 1.0f;
					if (Accelerometer.current.acceleration.ReadValue().z < 0) {
						acceleration *= -1.0f;
					}
				...
				case PassiveInputType.Mouse:
					horizontalTilt = (Mathf.Clamp ((Mouse.current.position.ReadValue().x / Screen.width), 0.0f, 1.0f) - 0.5f) * 2.0f;
					verticalTilt = (Mathf.Clamp ((Mouse.current.position.ReadValue().y / Screen.height), 0.0f, 1.0f) - 0.5f) * 2.0f;
					break;
	    ...
	    bool PointInAnyInterceptingFrame( Vector2 point ) {
	    	...
	    	for( i = 0; i < n; i++ ) {
	    		panel = panels[ i ];
				if ((panel.interceptInteraction && panel.camera.enabled && panel.camera.pixelRect.Contains( point )) || PanoplyCore.scene.disableNavigation) {
	    			result = true;
	    			break;
	    		}
	    	}	
	    ...
	    public void Update() {
            ...
            UnityEngine.InputSystem.EnhancedTouch.Touch touch = new UnityEngine.InputSystem.EnhancedTouch.Touch();
			...
			case DeviceType.Desktop:
				...
				if (Mouse.current.leftButton.wasPressedThisFrame) {
					...
					if (!PointInAnyInterceptingFrame (Mouse.current.position.ReadValue()) && !PanoplyCore.menu.isOpen) {
						gotInput = true;
						gestureStart = Mouse.current.position.ReadValue();
						ignoreCurrentGesture = false;

					} else {
						if (!PanoplyCore.menu.ScreenPointIsInsideMenu(Mouse.current.position.ReadValue())) {
					...
					if (!ignoreCurrentGesture) {
						...
						gestureDelta = Mouse.current.position.ReadValue() - gestureStart;
			...
			case DeviceType.Handheld:
				if (UnityEngine.InputSystem.EnhancedTouch.Touch.activeTouches.Count == swipeTouchCount) {

                        UnityEngine.InputSystem.TouchPhase dominantPhase = UnityEngine.InputSystem.TouchPhase.Canceled;

					for (i = 0; i < swipeTouchCount; i++) {
						touch = UnityEngine.InputSystem.EnhancedTouch.Touch.activeTouches [i];
						averagedPosition += UnityEngine.InputSystem.EnhancedTouch.Touch.activeTouches [i].screenPosition;
						if (dominantPhase == UnityEngine.InputSystem.TouchPhase.Canceled) {
							if (touch.phase == UnityEngine.InputSystem.TouchPhase.Began) {
								dominantPhase = touch.phase;
							} else if (touch.phase == UnityEngine.InputSystem.TouchPhase.Ended) {
								dominantPhase = touch.phase;
							}
						} else if (dominantPhase == UnityEngine.InputSystem.TouchPhase.Ended) {
							if (touch.phase == UnityEngine.InputSystem.TouchPhase.Began) {
								dominantPhase = touch.phase;
							}
						}
					...
					if (dominantPhase == UnityEngine.InputSystem.TouchPhase.Began) {
